...
def add(s1, s2):
    ...
    # Base case: both are simple filled sqares
    ...
    if type(s1) == type(s2) == int:
        ...
        # reutrn 0 if (s1 == 0 and s2 == 0) else 1

        return s1 | s2
    
    
    # Otherwise, if one is filled square and not a divided sqare,
    # make a divided sqaure of it

    if type(s1) == int:
        s1 = [s2, s2, s2, s2]

    # Recursively find sum of four quadrants of both squres
    #
